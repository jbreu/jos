...
void mini_print(const char *str) { write(1, str, strlen(str)); }

void mini_exit(int i) {}

// Function to allocate a lot of stack memory for test purposes
void allocate_stack_memory_recursive(int remaining) {
  ...
  allocate_stack_memory_recursive(remaining - sizeof(chunk));
}
...
void _start() {
  ...
  doom_set_file_io(fopen, fclose, fread, fwrite, fseek, ftell, feof);
