...
pub fn vga_plot_framebuffer(framebuffer: *const u8) {
    for x in 0..VGA_SCREEN_WIDTH {
        for y in 0..VGA_SCREEN_HEIGHT {
            ...
            unsafe {
                _SBUFFERS[_SBACK].buf[offset] = *(framebuffer.add(offset));
            }
        }
    }
}

#[instrument(fields(fid = 23))]
...
// Extracted from doom1.wad
const DOOM_PALETTE: [u8; 256 * 3] = [
...
fn vga_set_custom_palette(palette: &[u8; 256 * 3]) {
    ...
    for i in 0..256 {
        // Write the red component
        out_port_b(VGA_PALETTE_DATA, palette[i * 3] >> 2);
        // Write the green component
        out_port_b(VGA_PALETTE_DATA, palette[i * 3 + 1] >> 2);
        // Write the blue component
        out_port_b(VGA_PALETTE_DATA, palette[i * 3 + 2] >> 2);
    }
}
